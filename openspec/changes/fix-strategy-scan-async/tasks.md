...
- [ ] **1.1** 在 `compass/strategy/db.py` 新增 `get_latest_run(group_id)` 函数和 `cleanup_stale_runs()` 函数
...
- [ ] **1.2** 在 `compass/strategy/routes/signals.py` 新增 `GET /api/strategy/<group_id>/runs/latest` 路由
  - 调用 `db_helpers.get_latest_run(group_id)` 查询最新运行记录
  - 策略组不存在返回 404，无运行记录返回 200 + null

- [ ] **1.3** 在 `compass/strategy/app.py` 的 `init_strategy_engine()` 中调用 `cleanup_stale_runs()`
  - 在 `init_tables()` 和 `start_scheduler()` 之后调用
  - 异常仅 log warning，不影响引擎启动

## 组 2: 测试补充

- [ ] **2.1** 补充 `tests/test_strategy/test_async_scan.py` 测试用例
