...
class NumericSeries(TimeSeries):
    def __init__(self, series=[]):
        ...
        # self._series = series
        # if(isinstance(series,np.ndarray) and series.dtype == float):#保留小数点4位
        #     series = series.round(4)

        self._series = np.nan_to_num(series)#替换nan为0
    ...
    def __getitem__(self, index):
        ...
        #assert index < len(self.series)  #不能越界
        if(index >= len(self.series)):
            raise IndexError("out of index",index,len(self.series))
