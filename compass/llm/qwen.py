...
logger = logging.getLogger("compass.llm.qwen")


class QwenLLM(LLM):
    """Qwen（阿里 MaaS OpenAI 兼容端点）。

    配置源：config/llm_providers.json 的 active provider，
    与 datafactory / data-agent 的 providers.json 机制同构。
    """
    ...
    def __init__(self, api_key=None, base_url=None, model_id=None):
        ...
        super().__init__(
            ...
            model_id=model_id or active.get("model"),
        )
        ...
        for attempt in range(max_retries + 1):
            try:
                ...
                if attempt < max_retries:
                    wait = 2 ** attempt
                    logger.warning("Qwen request failed (%s), retry in %ds (attempt %d/%d)...",
                                   e, wait, attempt + 1, max_retries)
                    time.sleep(wait)
        logger.error("Qwen request failed after %d retries: %s", max_retries, last_err)
        return None
