...
class MACDDaily(object):
    ...
    def getData(self):
        ...
        sql_temp="select * from indicators_macd_daily where stock_code="+"\'"+self.code+"\' and record_time <=\'"+lastUpdateDate+"\';"
    ...
    def insert(self,MACD,DATETIME):
        ...
        for index in reversed(range(len(DATETIME))):
            try:
                if(math.isnan(m[index].value) or math.isnan(diff[index].value) or math.isnan(dea[index].value)):
                    print(m[index],diff[index],dea[index])
                    continue

                sql = self.db_insertsql(self.code,m[index],diff[index],dea[index],get_str_date_from_int(DATETIME[index].value/1000000))
                cur.execute(sql)
    ...
    def db_insertsql(self,stock_code,macd,diff,dea,record_time,):#返回的是插入语句
        sql_temp = '''
            replace into indicators_macd_daily (stock_code,macd,diff,dea,record_time) values (
            '''+"\'"+stock_code+"\',"+str(macd)+","+str(diff)+","+str(dea)+",\'"+record_time+"\');"
