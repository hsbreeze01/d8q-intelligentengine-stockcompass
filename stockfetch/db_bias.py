...
class BIASDaily(object):
    ...
    def getData(self):
        ...
        sql_temp="select * from indicators_bias_daily where stock_code="+"\'"+self.code+"\' and record_time <=\'"+lastUpdateDate+"\';"
    ...
    def insert(self,BIAS,DATETIME):
        ...
        for index in reversed(range(len(DATETIME))):
            try:
                if(math.isnan(j[index].value) or k[index].value >9999 or d[index].value >9999 or j[index].value >9999 ):
                    print(j[index])
                    continue
    ...
    def db_insertsql(self,stock_code,b1,b2,b3,record_time,):#返回的是插入语句
        sql_temp = '''
            replace into indicators_bias_daily (stock_code,bias_1,bias_2,bias_3,record_time) values (
