...
class BOLLDaily(StockDBBase):
    """BOLL daily indicator — inherits StockDBBase for pooled DB access."""
    ...
    def getData(self):
        """Return a DataFrame of BOLL indicators up to the last known daily date."""
        ...
        with self:
            _, rows = self._query_all(
                "SELECT * FROM indicators_boll_daily "
    ...
    def insert(self, BOLL, DATETIME):
        """Batch-insert BOLL indicator values, skipping NaN."""
        up = BOLL[0]
        mid = BOLL[1]
        low = BOLL[2]
        ...
        with self:
            for index in reversed(range(len(DATETIME))):
                try:
                    ...
                    self._execute_many(
                        "REPLACE INTO indicators_boll_daily "
                        "(stock_code, upper_v, mid_v, lower_v, record_time) "
                        "VALUES (%s, %s, %s, %s, %s)",
